...
## 本脚本搬运并模仿 liuqitoday
dir_config=/ext_overlay/docker/ql/data/config
dir_script=/ext_overlay/docker/ql/data/scripts
dir_repo=/ext_overlay/docker/ql/data/repo
dir_deps=/ext_overlay/docker/ql/data/deps
...
# 获取有效 extra.sh 链接
get_valid_extra() {
    extra_list=(https://git.metauniverse-cn.com/https://raw.githubusercontent.com/yanyuwangluo/VIP/main/Tasks/ext_overlay/docker/qlrepo/extra.sh)
...
# 将 ql extra 添加到定时任务
add_ql_extra() {
    if [ "$(grep -c "ql\ extra" /ext_overlay/docker/ql/data/config/crontab.list)" != 0 ]; then
        echo "您的任务列表中已存在 task:ql extra"
    else
        echo "开始添加 task:ql extra"
        # 获取token
        token=$(cat /ext_overlay/docker/ql/data/config/auth.json | jq --raw-output .token)
...
# 添加定时任务 自动更新模板
add_curl_sample() {
    if [ "$(grep -c "config.sample.sh" /ext_overlay/docker/ql/data/config/crontab.list)" != 0 ]; then
        echo "您的任务列表中已存在 task:自动更新模板"
    else
        echo "开始添加 task:curl config.sample.sh"
        # 获取token
        token=$(cat /ext_overlay/docker/ql/data/config/auth.json | jq --raw-output .token)
        curl -s -H 'Accept: application/json' -H "Authorization: Bearer $token" -H 'Content-Type: application/json;charset=UTF-8' -H 'Accept-Language: zh-CN,zh;q=0.9' --data-binary '{"name":"自动更新模板","command":"curl -L https://git.metauniverse-cn.com/https://raw.githubusercontent.com/yanyuwangluo/VIP/main/Conf/Qinglong/config.sample.sh -o /ext_overlay/docker/ql/data/sample/config.sample.sh && cp -rf /ext_overlay/docker/ql/data/sample/config.sample.sh /ext_overlay/docker/ql/config","schedule":"45 6,18 * * *"}' --compressed 'http://127.0.0.1:5600/api/crons?t=1697961933000'
    fi
}
run_curl_sample() {
    curl -sL $valid_url -o /ext_overlay/docker/ql/data/sample/config.sample.sh && cp -rf /ext_overlay/docker/ql/data/sample/config.sample.sh /ext_overlay/docker/ql/data/config
}
if [ "${all}" = 1 ]; then
